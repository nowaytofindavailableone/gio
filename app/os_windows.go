...
func (w *window) Configure(options []Option) {
	...
	case Windowed:
		...
		// Handle position
		if w.config.Position.X != 0 || w.config.Position.Y != 0 {
			// Use specified position
			x = int32(w.config.Position.X)
			y = int32(w.config.Position.Y)
		} else {
			// Center on screen
			screenWidth := int32(windows.GetSystemMetrics(_SM_CXSCREEN))
			screenHeight := int32(windows.GetSystemMetrics(_SM_CYSCREEN))
